...
/// Implementation of `Component` and `ComponentProver` for the Input `,` instruction component.
///
/// It targets the `SimdBackend` from the Stwo constraint framework, with a fallback
/// on `CpuBakend` for small traces.
pub type InputInstructionComponent = FrameworkComponent<InputInstructionEval>;

/// The AIR for the [`JumpIfZeroComponent`].
///
/// Constraints are defined through the [`FrameworkEval`]
/// provided by the constraint framework of Stwo.
pub struct InputInstructionEval {
...
impl FrameworkEval for InputInstructionEval {
    ...
    /// Defines the AIR for the [`InputInstructionComponent`].
    ///
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // `ci` is either 0 (dummy row), or equal to the Input instruction `,`.
        let input_instruction = ci.clone() - InstructionType::ReadChar.to_base_field().into();

        eval.add_constraint(ci.clone() * input_instruction);
...
mod test {
    use crate::components::processor::{
        instructions::{
            input_component::InputInstructionEval,
            table::{interaction_trace_evaluation, InputInstructionTable},
        },
    ...
    #[test]
    fn test_input_instruction_constraints() {
        ...
        let table = InputInstructionTable::from(&trace_vm);
        ...
        // Get the Input `,` AIR evaluator
        let input_instruction_eval = InputInstructionEval::new(&claim, processor_lookup_elements);
