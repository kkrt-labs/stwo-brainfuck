...
/// Implementation of `Component` and `ComponentProver` for the Left `<` instruction component.
///
/// It targets the `SimdBackend` from the Stwo constraint framework, with a fallback
/// on `CpuBakend` for small traces.
pub type LeftInstructionComponent = FrameworkComponent<LeftInstructionEval>;

/// The AIR for the [`LeftInstructionComponent`].
///
/// Constraints are defined through the [`FrameworkEval`]
/// provided by the constraint framework of Stwo.
pub struct LeftInstructionEval {
...
impl FrameworkEval for LeftInstructionEval {
    ...
    /// Defines the AIR for the [`LeftInstructionComponent`].
    ///
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // `ci` is either 0 (dummy row), or equal to the Left (memory shift left) instruction `<`.
        let left_instruction = ci.clone() - InstructionType::Left.to_base_field().into();

        eval.add_constraint(ci.clone() * left_instruction);
...
mod test {
    use crate::components::processor::{
        instructions::{
            left_component::LeftInstructionEval,
            table::{interaction_trace_evaluation, LeftInstructionTable},
        },
    ...
    #[test]
    fn test_left_instruction_constraints() {
        ...
        let table = LeftInstructionTable::from(&trace_vm);
        ...
        // Get the Left `<` AIR evaluator
        let input_instruction_eval = LeftInstructionEval::new(&claim, processor_lookup_elements);
