...
/// Implementation of `Component` and `ComponentProver` for the Output `.` instruction component.
///
/// It targets the `SimdBackend` from the Stwo constraint framework, with a fallback
/// on `CpuBakend` for small traces.
pub type OutputInstructionComponent = FrameworkComponent<OutputInstructionEval>;

/// The AIR for the [`OutputInstructionComponent`].
///
/// Constraints are defined through the [`FrameworkEval`]
/// provided by the constraint framework of Stwo.
pub struct OutputInstructionEval {
...
impl FrameworkEval for OutputInstructionEval {
    ...
    /// Defines the AIR for the [`OutputInstructionComponent`].
    ///
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // `ci` is either 0 (dummy row), or equal to the Output instruction `.`.
        let output_instruction = ci.clone() - InstructionType::PutChar.to_base_field().into();

        eval.add_constraint(ci.clone() * output_instruction);
...
mod test {
    use crate::components::processor::{
        instructions::{
            output_component::OutputInstructionEval,
            table::{interaction_trace_evaluation, OutputInstructionTable},
        },
    ...
    #[test]
    fn test_output_instruction_constraints() {
        ...
        let table = OutputInstructionTable::from(&trace_vm);
        ...
        // Get the Output `.` AIR evaluator
        let input_instruction_eval = OutputInstructionEval::new(&claim, processor_lookup_elements);
        ...
        assert_constraints(
            ...
            |eval| {
                input_instruction_eval.evaluate(eval);
            },
