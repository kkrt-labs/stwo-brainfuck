...
impl FrameworkEval for JumpIfZeroEval {
    ...
    /// Defines the AIR for the [`JumpIfZeroComponent`].
    ///
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // `ci` is either 0 (dummy row), or equal to the JumpIfZero instruction `[`.
        let jump_if_zero_instruction =
            ci.clone() - InstructionType::JumpIfZero.to_base_field().into();

        eval.add_constraint(ci.clone() * jump_if_zero_instruction);
        ...
        // Jump to `ni + 1` if `mv == 0`. Otherwise, skip two.
        eval.add_constraint(
            (d.clone() - E::F::one()) *
                (mv.clone() * (next_ip.clone() - ip.clone() - BaseField::from(2).into()) +
                    is_mv_zero * (next_ip - (ni.clone() + E::F::one()))),
        );
...
mod test {
    use crate::components::processor::{
        instructions::jump::{
            jump_if_zero_component::JumpIfZeroEval,
            table::{interaction_trace_evaluation, JumpIfZeroTable},
        },
    ...
    #[test]
    fn test_jump_if_zero_constraints() {
        ...
        let table = JumpIfZeroTable::from(&trace_vm);
        ...
        let jump_if_zero_eval = JumpIfZeroEval::new(&claim, processor_lookup_elements);
        ...
        assert_constraints(
            ...
            |eval| {
                jump_if_zero_eval.evaluate(eval);
            },
