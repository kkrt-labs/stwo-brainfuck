...
/// Implementation of `Component` and `ComponentProver` for the Minus `-` instruction component.
/// It targets the `SimdBackend` from the Stwo constraint framework, with a fallback
/// on `CpuBakend` for small traces.
pub type MinusInstructionComponent = FrameworkComponent<MinusInstructionEval>;

/// The AIR for the [`MinusInstructionComponent`].
///
/// Constraints are defined through the [`FrameworkEval`]
/// provided by the constraint framework of Stwo.
pub struct MinusInstructionEval {
...
impl FrameworkEval for MinusInstructionEval {
    ...
    /// Defines the AIR for the [`MinusInstructionComponent`].
    ///
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // `ci` is either 0 (dummy row), or equal to the Minus (memory decrement) instruction `-`.
        let minus_instruction = ci.clone() - InstructionType::Minus.to_base_field().into();

        eval.add_constraint(ci.clone() * minus_instruction);
...
mod test {
    use crate::components::processor::{
        instructions::{
            minus_component::MinusInstructionEval,
            table::{interaction_trace_evaluation, MinusInstructionTable},
        },
    ...
    #[test]
    fn test_minus_instruction_constraints() {
        ...
        // Get the Minus `-` AIR evaluator
        let input_instruction_eval = MinusInstructionEval::new(&claim, processor_lookup_elements);
